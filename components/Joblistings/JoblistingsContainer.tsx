...
const ListRenderer = props => <>
    ...
    <InfiniteScroll element="div" hasMore={props.relay.hasMore()} loadMore={() => {
    if (props.loading || !props.relay.hasMore() || props.relay.isLoading()) {
      return;
    }

    props.loadingStart();
    props.relay.loadMore(30, // Fetch the next 30 feed items
    error => {
      props.loadingEnd();
    });
  }} threshold={50} loader={<LoadingIndicator hideText noMargin />}>
      <JoblistingGrid>
        {props.root && props.root.joblistings.edges.map(({
        node
      }) => <CompanyElement key={node.id}>
              <Link key={node.id} href={'/jobb/[slug]'} as={`/jobb/${node.slug}`}>
                <a>
                  <CompanyImage src={node.company.logo || '/static/itdagene-gray.png'} />
                  <h3 style={{
              fontWeight: 'normal',
              fontSize: 20,
              lineHeight: '24px',
              ...
              textAlign: 'center'
                  ...
                  <div style={{ color: 'gray', textAlign: 'center' }}>
                    {jobTypeOptions.find(el => el.value === node.type.toLowerCase()).label}{' '}
                    @ {node.company.name}
                  </div>
                  <div style={{
              color: 'gray',
              ...
              margin: '3px'
                    ...
                    {node.deadline ? 'Frist: ' + dayjs(node.deadline).format(`D. MMMM ${isCurrentYear(node.deadline) ? '' : 'YYYY'}`) : 'Løpende søknadsfrist'}
                  </div>

                  <div style={{ color: 'gray', textAlign: 'center' }}>
                    {node.towns.length > 3 ? `${node.towns[0].name}, ${node.towns[1].name}, ${node.towns[2].name.slice(0, 3)}...` : joinValues(node.towns.map(({
                name
              }) => name))}
                  </div>
                </a>
        ...
        {props.root && props.root.joblistings.edges.length === 0 && <h2> Ingen annonser :( </h2>}
      </JoblistingGrid>
    </InfiniteScroll>
  </>;

export const JoblistingsList = createPaginationContainer(ListRenderer, {
  root: graphql`
