...
const Content = styled('div')`
  ...
  ${({
  responsive = false
}: {responsive?: boolean;}) => responsive && css`
...
const pageview = url => {
  window.__GA_TRACKING_ID__ && window.gtag('config', window.__GA_TRACKING_ID__, {
    page_location: url
  });
};
...
export type LayoutSettings<T> = {
  ...
  customOpengraphMetadata?: (props: {props: T;error: Error | null | undefined;}) => {
...
export type ContentRenererProps<T> = {props: T;error: Error | null | undefined;};
...
export const Layout = <T>({
  ...
  if (!props && !noLoading) return <Wrapper>
        <CustomOpengraphRenderer />
        <HeaderMenu />
        <Content center>
          <LoadingIndicator />
  ...
  return <div>
      <Wrapper>
        <HeaderMenu />
        {customOpengraphMetadata ? <CustomOpengraphRenderer object={customOpengraphMetadata && props ? customOpengraphMetadata({ props, error }) : null} /> : metadata ? <OpengraphFragmentRenderer metadata={metadata} /> : <CustomOpengraphRenderer />}
        <Content center={shouldCenter} responsive={responsive}>
          {props && ContentRenderer ? <ContentRenderer error={error} props={props} /> : null}
