...
import JoblistingsContainer, { query, JoblistingsList } from "../components/Joblistings/JoblistingsContainer";
...
class Index extends React.Component<RenderProps, State> {
  ...
  render() {
    ...
    return <Layout customOpengraphMetadata={() => ({
      title: 'Jobbannonser'
    })} props noLoading responsive>
        <JoblistingsContainer environment={environment} variables={variables}>
          <JoblistingsList environment={environment} loading={this.state.loading} loadingStart={this.loadingStart} loadingEnd={this.loadingEnd} root={props} />
...
export default withData(withRouter(Index), {
  ...
  variables: router => ({
    ...
    towns: parseTowns(router.query) && parseTowns(router.query).map(el => el.value),
