...
import { standsPlacement, companiesDay1TopRight, companiesDay1BottomLeft } from "./standsData";
      ...
      >
        {splitIntoColumns(
          ...
          <div key={colIndex} style={{ display: "flex", flexDirection: "column", gap: "0.1rem" }}>
            {col.map((numberStr) => {
              const standIndex = parseInt(numberStr, 10);
              const companyName = companiesDay1BottomLeft[standIndex - companyNumbersRight.length - 1];
              return (
                <CompaniesText
                  key={standIndex}
                  isHighlighted={hoveredStand === standIndex || hoveredClickStand === standIndex}
