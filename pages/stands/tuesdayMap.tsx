...
const MondayMap = () => {
  ...
  const companyNumbersRight = companiesDay2TopRight.map(
    (_, index) => index + 1
  );
  const columnsOverlayRight = splitIntoColumns(
    companyNumbersRight.map(String),
    numColsOverlay
  );
        ...
        {splitIntoColumns(
          ...
          >
            {col.map((numberStr) => {
              ...
              const companyName =
                companiesDay2BottomLeft[
                  standIndex - companyNumbersRight.length - 1
                ];
              return (
                <CompaniesText
                  key={standIndex}
